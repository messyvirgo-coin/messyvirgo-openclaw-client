...
compose_base() {
  ...
  # macOS: Docker Desktop often fails binding to 127.0.0.1. Compose merges (concatenates) ports,
  # so secure's 127.0.0.1 would still be tried. Use base + macos only so we get 0.0.0.0:18789/18790.
  if is_macos && [[ -f "$ROOT_DIR/docker-compose.macos.yml" ]]; then
    files+=(-f "$ROOT_DIR/docker-compose.macos.yml")
  else
    files+=(-f "$ROOT_DIR/docker-compose.secure.yml")
  fi
