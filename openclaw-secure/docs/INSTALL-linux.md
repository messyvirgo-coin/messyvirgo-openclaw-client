...
- with `./secure-client/scripts/up.sh`, usually use `http://172.17.0.1:8000/mcp`
- with `./secure-client/scripts/up-linux-hostnet.sh`, use `http://localhost:8000/mcp`
...
- because `setup.sh` starts the gateway, you can go directly to step 3 without running `./secure-client/scripts/up.sh`
...
```bash
./secure-client/scripts/cli.sh devices list
./secure-client/scripts/cli.sh devices approve <requestId>
```

How to get `<requestId>`:

1. Run `./secure-client/scripts/cli.sh devices list`
2. Find the pending pairing entry for your browser/device
3. Copy its `requestId` value
4. Run `./secure-client/scripts/cli.sh devices approve <requestId>` with that exact value
...
- `./secure-client/scripts/up.sh` -> usually `MESSY_VIRGO_MCP_URL=http://172.17.0.1:8000/mcp`
- `./secure-client/scripts/up-linux-hostnet.sh` -> `MESSY_VIRGO_MCP_URL=http://localhost:8000/mcp`
...
```bash
sudo ./secure-client/scripts/setup.sh
```
...
2. Approve the pending device with `./secure-client/scripts/cli.sh devices list` and `./secure-client/scripts/cli.sh devices approve <requestId>`
